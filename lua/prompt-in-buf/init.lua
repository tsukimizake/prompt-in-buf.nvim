...
local function create_popup_buffer()
  ...
  local function insert_to_original()
    ...
    if content and content ~= '' then
      ...
      vim.api.nvim_buf_set_text(original_buf, cursor_line - 1, cursor_col, cursor_line - 1, cursor_col, vim.split(content, '\n'))
    end
  end
  
  vim.keymap.set('n', '<CR>', insert_to_original, { buffer = buf, noremap = true, silent = true })
